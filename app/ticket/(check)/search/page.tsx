  ...
  return (
    ...
    >
      ...
      <div className="h-[238px] w-full pad:rounded-b-xl pad:border pad:border-gray-15 flex flex-col mx-auto px-4 pad:px-12 dt:px-[102px]">
        <p className="mt-8 pad:mt-[72px] font-medium leading-[30px] text-[18px] pad:text-[20px]">
          예매 번호 입력{' '}
        </p>
        <div className="flex flex-row gap-4 pad:gap-6 mt-4 items-center w-full">
          <input
            ...
            className="flex h-12 w-full pad:w-[588px] rounded-xl border border-gray-15 px-4 py-3 text-[16px] font-light focus:outline-none focus:border-primary-40 focus:outline-[1px]"
          />
          <button
            onClick={handleSearchReservation}
            className={`w-[180px] h-[59px] hidden pad:flex flex-shrink-0 text-center justify-center items-center mx-auto rounded-xl text-[18px] font-normal ${buttonClass} ${reservationId.length === 10 ? '' : 'cursor-not-allowed'}`}
          ...
          <button
            onClick={handleSearchReservation}
            className={`pad:hidden w-[70px] h-[48px] flex flex-shrink-0 text-center justify-center items-center mx-auto rounded-xl text-[16px] font-normal ${buttonClass} ${reservationId.length === 10 ? '' : 'cursor-not-allowed'}`}
            disabled={!isButtonEnabled}
          >
            조회
          </button>
      ...
      <Link
        href="/ticket/"
        className="fixed bottom-8 left-1/2 transform -translate-x-1/2 pad:relative pad:mt-20 pad:left-0 pad:-translate-x-0 w-[328px] pad:w-[384px] h-[59px] flex flex-shrink-0 text-center justify-center items-center mx-auto rounded-xl text-[18px] font-normal text-gray-60 bg-gray-5"
      >
        예매 페이지로 돌아가기
      </Link>
      <NotFoundModal
