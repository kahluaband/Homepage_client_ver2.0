...
const TwoOptionBox: React.FC<twoOptionProps> = ({ option1, option2, seletion, className } ) => {
    ...
    <div className={`flex flex-row gap-4 pad:flex-col ${className}`}>
        <div onClick={(e) => setSelected(option1)} className={`mt-2 flex items-center justify-between px-4 h-12 w-[156px] pad:w-[282px] rounded-xl border ${selected === option1 ? "border-primary-50 text-primary-50" : "border-gray-40 text-gray-90"} bg-gray-0 text-[16px] font-normal leading-6 text-center`}>
            {option1}
