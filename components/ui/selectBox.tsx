    ...
    ({ className, name, alt, state, ...props }, ref) => {
        return (
            <button
                ...
                onClick={props.onClick} type="button" ref={ref}
            >
                <p className={`text-start flex items-center font-[16px] text-medium leading-6 w-[282px] h-[48px] ${state ? "text-primary-50" : "text-gray-80"}`}>
                    {name}
                </p>
                {state && <Image src="/image/ticket/check.svg" width={20} height={20} alt={alt || "text"} className="" />}
            </button>
        );
    }
);
